...
class ToontownControlManager(ControlManager.ControlManager):
    wantWASD = base.wantWASD # the toggle option is in the toonbase file
    ...
    def disableWASD(self):# disables movement when chat is opened
        if self.wantWASD:
            self.forceTokens=[
                inputState.force(
    ...
    def enableWASD(self):# enables movement when chat is closed
        if self.wantWASD:
            if self.forceTokens:
