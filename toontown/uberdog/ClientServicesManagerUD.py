...
class UnloadAvatarFSM(OperationFSM):
    ...
    def enterUnloadAvatar(self):
        ...
        # Tell TTJFriendsManager somebody is logging off:
...
class ClientServicesManagerUD(DistributedObjectGlobalUD):
    ...
    def rejectKey(self, sender, issue, securityIssue=True, dumb=True):
        initialString = "While trying to remotely connect to Toontown Journey, " + issue + '. You will need to fix this issue before you can login to Toontown Journey.'
        if securityIssue:
            initialString = "While trying to remotely connect to Toontown Journey, " + issue + '. This appears to be an accidential security issue while connecting. You will need to fix this issue before you can login to Toontown Journey.'
        if dumb:
            initialString = "While trying to remotely connect to Toontown Journey, " + issue + '. This may be a faulty security bypass attempt due to the issue. You will need to fix this issue before you can login to Toontown Journey.'
        if securityIssue and dumb:
            initialString = "While trying to remotely connect to Toontown Journey, " + issue + '. This appears to be an accidential security issue while connecting. This may be a faulty security bypass attempt due to the issue. You will need to fix this issue before you can login to Toontown Journey.'
    ...
    def login(self, cookie, authKey):
        ...
        # Ensure that our authentication key is the correct size:
        if len(authKey) > 22:
        ...
        if not authKey in ['ttjsecretkey3234523423', '3243254323yektercesjtt']:
            if not authKey in 'ttjsecretkey3234523423':
                ...
                self.rejectKey("the specified authentication key did not match up to the server's set key", dumb=False)
            else:
                self.rejectKey("you reversed the server's set authentication key", securityIssue=False, dumb=False)
