...
class FFSafeZoneLoader(SafeZoneLoader.SafeZoneLoader):
    ...
    def __loadPythonImplementation(self, task):
        ...
        if safeZone:
            ...
            dgaFountain = ToonLandRepository.ToonLandRepository.createDistributedObject(
             ...
             zoneId=PlaygroundGlobals.FUNNY_FARM, doId=ToonLandStaticIDs.DISTRIBUTED_GEYSER_FOUNTAIN_ID,
            ...
            trashCan = ToonLandRepository.ToonLandRepository.createDistributedObject(
             ...
             zoneId=PlaygroundGlobals.FUNNY_FARM, doId=ToonLandStaticIDs.DISTRIBUTED_HICCUP_TRASH_CAN_ID,
             sendFormatGenerate=False)
            trashCan.reparentTo(safeZone)
            trashCan.setPosHpr(72, 30, 10.025, -812, 0, 0)
            trashCan.setScale(1.1)
            for tn in range(len(PicnicBasketPositions)):
                self.createDistributedPicnicBasket((tn + 1), *PicnicBasketPositions[tn])
            return Task.done
    ...
    def createDistributedHouse(self, houseIndex):
        ...
        distObj.doId = ((houseIndex + 1) << 20) # We don't need a doId. So we create a fake 32-bit doId.
        ToonLandRepository.ToonLandRepository.doId2do[distObj.doId] = distObj
        distObj.generateInit()
        distObj.generate()
        distObj.setLocation(base.localAvatar.defaultShard, PlaygroundGlobals.FUNNY_FARM)
    ...
    def createDistributedPicnicBasket(self, tn, *poshpr):
        ...
        ToonLandRepository.ToonLandRepository.createDistributedObject(distObj=distObj,
         ...
         doId=eval('ToonLandStaticIDs.DISTRIBUTED_FF_PICNIC_BASKET_%d' % tn), sendFormatGenerate=False)
